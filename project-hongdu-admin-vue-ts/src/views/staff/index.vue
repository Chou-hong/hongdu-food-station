        ...
        <el-table-column label="操作" width="160" align="center">
          <template slot-scope="scope">
            <el-button
              ...
              @click="addStaffHandle(scope.row.id, scope.row.username)"
            >
              修改
            </el-button>
            <el-button
...
import { getStaffList, enableOrDisableStaff } from '@/api/staff'
...
export default class extends Vue {
  ...
  // 添加
  private addStaffHandle(st: string, username: string) {
    if (st === 'add') {
      ...
      this.$router.push({ path: '/staff/add', query: { id: st } })
    }
  }
  ...
  private statusHandle(row: any) {
    ...
    this.$confirm('确认调整该账号的状态?', '提示', {
      ...
      enableOrDisableStaff({ id: this.id, status: !this.status ? 1 : 0 })
        .then((res) => {

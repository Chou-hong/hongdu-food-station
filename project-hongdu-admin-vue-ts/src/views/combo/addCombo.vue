...
import AddDish from './components/AddProduct.vue'
import { queryComboById, addCombo, editCombo } from '@/api/combo'
...
export default class extends Vue {
  ...
  private async init() {
    queryComboById(this.$route.query.id).then(res => {
      if (res && res.data && res.data.code === 1) {
        ...
        this.checkList = res.data.data.comboItemes
        this.dishTable = res.data.data.comboItemes.reverse()
  ...
  public submitForm(formName: any, st: any) {
    ;(this.$refs[formName] as any).validate((valid: any) => {
      if (valid) {
        ...
        prams.comboItemes = this.dishTable.map((obj: any) => ({
          copies: obj.copies,

<template>
  <div class="selectInput">
    ...
    />
    <div v-if="mak && productSpecsData.length" class="flavorSelect">
      <span
        v-for="(it, ind) in productSpecsData"
        :key="ind"
        class="items"
        @click="checkOption(it, ind)"
        >{{ it.name }}</span
      >
      <span v-if="productSpecsData == []" class="none">无数据</span>
    </div>
  </div>
</template>
...
export default class extends Vue {
  ...
  @Prop({ default: [] }) private productSpecsData!: []

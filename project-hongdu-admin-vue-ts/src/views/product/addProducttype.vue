    ...
    <div :key="restKey"
      ...
      <el-form ref="ruleForm"
        ...
        <el-form-item label="口味做法配置:">
          <el-form-item>
            <div class="flavorBox">
              <span v-if="productSpecs.length == 0"
                    class="addBut"
                    @click="addFlavore">
                + 添加口味</span>
              <div v-if="productSpecs.length != 0"
                ...
                <div class="cont">
                  <div v-for="(item, index) in productSpecs"
                       :key="index"
                       class="items">
                    <div class="itTit">
                ...
                <div v-if="
                       !!this.leftDishFlavors.length &&
                         this.productSpecs.length < this.productSpecsData.length
                     "
                     class="addBut"
                     @click="addFlavore">
                  添加口味
...
export default class extends Vue {
  ...
  private productSpecsData: any[] = [] //原始口味数据
  private productSpecs: any[] = [] //待上传口味的数据
  ...
  getLeftDishFlavors() {
    let arr = []
    this.productSpecsData.map(item => {
      if (
        this.productSpecs.findIndex(item1 => item.name === item1.name) === -1
      ) {
        arr.push(item)
      }
    })
  ...
  private selectHandle(val: any, key: any, ind: any) {
    const arrDate = [...this.productSpecs]
    const index = this.productSpecsData.findIndex(item => item.name === val)
    arrDate[key] = JSON.parse(JSON.stringify(this.productSpecsData[index]))
    this.productSpecs = arrDate
  }
  ...
  // 按钮 - 添加口味
  private addFlavore() {
    this.productSpecs.push({ name: '', value: [] }) // JSON.parse(JSON.stringify(this.productSpecsData))
  }

  // 按钮 - 删除口味
  private delFlavor(name: string) {
    let ind = this.productSpecs.findIndex(item => item.name === name)
    this.productSpecs.splice(ind, 1)
  }

  // 按钮 - 删除口味标签
  private delFlavorLabel(index: number, ind: number) {
    this.productSpecs[index].value.splice(ind, 1)
  }
  ...
  private keyDownHandle(val: any) {
    ...
    if (val.target.innerText.trim() != '') {
      this.productSpecs[this.index].flavorData.push(val.target.innerText)
  ...
  private submitForm(formName: any, st: any) {
    ;(this.$refs[formName] as any).validate((valid: any) => {
      ...
      if (valid) {
        ...
        // params.flavors = this.productSpecs
        params.status =
          this.actionType === 'add' ? 0 : this.ruleForm.status ? 1 : 0
        // params.price *= 100
        params.categoryId = this.ruleForm.categoryId
        params.flavors = this.productSpecs.map(obj => ({
          ...obj,
          value: JSON.stringify(obj.value)
        }))
        delete params.productSpecs
